...
public:
	...
	static IndexedTriangleList<V> MakeTesselated(int latDiv, int longDiv)
	{
		...
		const auto iNorthPole = (Index)vertices.size();
		vertices.emplace_back();
		dx::XMStoreFloat3(&vertices.back().pos, base);
		const auto iSouthPole = (Index)vertices.size();
		vertices.emplace_back();
		dx::XMStoreFloat3(&vertices.back().pos, dx::XMVectorNegate(base));

		const auto calcIdx = [latDiv, longDiv](Index iLat, Index iLong)
		{ return iLat * longDiv + iLong; };
		std::vector<Index> indices;
		for (Index iLat = 0; iLat < latDiv - 2; iLat++)
		{
			for (Index iLong = 0; iLong < longDiv - 1; iLong++)
			{
